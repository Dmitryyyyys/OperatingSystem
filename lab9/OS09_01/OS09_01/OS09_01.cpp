...
#include<iomanip>
using namespace std;
#define FILE_PATH L"C:/labs/ОС/OS09_01.txt"
...
LPCWSTR getFileType(HANDLE file)
{
	...
	BOOL printFileInfo(LPWSTR path)
	{
		...
		if (fResult)
		{
			...
			cout << "\nCreate time:\t" << setfill('0') << setw(2) << sysTime.wHour + 3 << ':' << setw(2) << sysTime.wMinute << ':' << setw(2) << sysTime.wSecond;
